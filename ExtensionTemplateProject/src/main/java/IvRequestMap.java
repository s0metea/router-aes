...
class IvRequestMap {
    ...
    static void rememberPlaintext(String ivB64, String plaintext) {
        if (ivB64 == null || ivB64.isEmpty() || plaintext == null) return;
        IV_TO_PLAINTEXT.put(ivB64, plaintext);
    }

    static String takePlaintext(String ivB64) {
        if (ivB64 == null) return null;
    ...
    static void clearForIv(String ivB64) {
        ...
        IV_TO_PLAINTEXT.remove(ivB64);
    }

    static void clearForMessageId(int messageId) {
        MSG_TO_REQ.remove(messageId);
        IV_TO_MSG.entrySet().removeIf(e -> e.getValue() == messageId);
    }
}